...
- A topic name (e.g. `locations`, `characters`, `themes`) â€” use as the initial topic and skip the topic prompt in Step 1.
- `character [name]` (e.g. `character mira`) â€” pre-fill `CHARACTER_NAME` and skip that sub-prompt.
- `challenge` â€” activate **Challenge Mode** after topic resolution: questions stress-test the loaded file's decisions rather than fill gaps (see Step 4a).
- A session length flag: `quick`, `standard`, or `deep` â€” skip the session length prompt in Step 3.
...
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  BRAINSTORM â€” Topic Selection
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”

  What do you want to brainstorm?

   1  spec           â€” Story concept, logline, dramatic question
   2  plan           â€” Story structure, acts, chapter map
   3  characters     â€” Cast of characters, arcs, dynamics
   4  character      â€” A single specific character (you name them)
   5  themes         â€” Central themes, motifs, symbolic layer
   6  world-building â€” Setting rules, geography, culture, history
   7  locations      â€” Specific places, sensory anchors, atmosphere
   8  pov            â€” Narrative perspective, voice strategy
   9  research       â€” Factual grounding, open questions
  10  timeline       â€” Story chronology, scene durations, gaps
  11  series         â€” Series arc, canon, book-to-book continuity
  12  glossary       â€” Invented terms, proper nouns, spelling rules
...
| `(custom)` | No canonical file â€” treat as a blank-slate topic |
...
- **Prior notes found**: Display:
  ```
  â†© Prior brainstorm notes found for [topic] ([N] insights).
    (r) Resume â€” load prior notes; new questions will build on them
    (n) New session â€” start fresh (prior file kept on disk, not loaded)
  ```
...
- **File found**: Read the full file content. Set `SESSION.has_context = true`. Display:
  ```
  âœ“ Found [filename] â€” I'll use this as context for the session.
  ```
  Summarise the document in 2â€“3 bullets (key decisions already locked in) so the user can confirm AI read it correctly before questions begin.
  ...
  - **Additional index context** (if `.specify/index/` exists): query for related project content scattered across files other than the canonical file for this topic:
    ...
    ```
    Surface any connections, constraints, or prior decisions from `character`, `world`, `draft`, `research`, or `series` doc types that are relevant to the brainstorm topic. Present any conflicts with the loaded canonical file as Change Candidates for the user to resolve â€” do not silently override.

- **File not found**: Set `SESSION.has_context = false`. Display:
  ```
  âœ— No [filename] found â€” I'll start from scratch with foundational questions.
  ```

Also load `.specify/memory/constitution.md` if it exists â€” it applies to all topics as background context.
...
## Step 3 â€” Session Start

**Session length** â€” if not already set via argument, ask:
```
How deep do you want to go?
  q  quick    â€” ~5 questions, highest-priority gaps only
  s  standard â€” ~10 questions, core coverage  (default)
  d  deep     â€” unlimited, follow every thread
```
...
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  BRAINSTORM SESSION
  Topic    : [topic]  [âœ“ loaded from file | âœ— new topic]
  Mode     : [With context | Blank slate | Challenge]
  Depth    : [Quick Â· Standard Â· Deep]
  Resuming : [Yes â€” [N] prior insights loaded | No]  â† omit if not resuming
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
Commands available at any time:
  done        â€” end session and choose what to do with notes
  cancel / q  â€” discard everything and exit
  switch      â€” change to a different topic (your current insights are preserved)
  summary     â€” show a running summary of insights so far
  !skip       â€” skip this question and move to the next one
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
```

Initialise an internal **Insight Log** â€” an ordered list of brainstorm findings that grows throughout the session. Each entry records:
- The question asked
...
## Step 4 â€” Brainstorm Loop

The loop runs until the user types `done`, `cancel`, or `switch`.

### 4a â€” Select the next question

**Depth gate**: if `SESSION.depth` is `quick` and N â‰¥ 5, or `standard` and N â‰¥ 10, surface a natural stopping point before asking another question:
```
...
**Challenge mode**: if `SESSION.mode` is `Challenge`, invert the normal priority. Lead with questions that directly challenge the most confident decisions in the loaded file. Frame each as: *â€œYour [element] is [current value] â€” what is the strongest argument that this is wrong?â€* Do not fabricate a counter-argument; only ask the question.

**Normal priority order**:

1. **Tension probe**: if the userâ€™s previous answer introduced a contradiction â€” with the loaded file, a prior answer in this session, or the loaded `SESSION.prior_log` (if resuming) â€” probe that tension first.
...
Never repeat a question already asked in this session or present in `SESSION.prior_log`. Never ask about something definitively answered in the loaded file â€” except in Challenge mode.

### 4b â€” Ask the question
...
(done Â· cancel Â· switch Â· summary Â· !skip)
```

Where `{N}` is the sequential question number in this session.

After the user answers:
- Acknowledge in 1â€“2 sentences. The acknowledgement **must name at least one** of:
  - The **narrative function** this element serves (e.g. â€œThat gives the wound a physical anchor in Act 1â€)
  - The **tension it creates** with another locked decision (e.g. â€œThis sits in tension with the timeline â€” she canâ€™t have been in both placesâ€)
  - A **specific scene beat or document section** it will affect (e.g. â€œThis reshapes the midpoint directlyâ€)
  Do not produce generic filler (â€œInteresting!â€, â€œThatâ€™s a great pointâ€, â€œGood thinkingâ€).
...
- If the answer conflicts with the loaded file or a prior answer in this session:
  ```
  âš   Conflict with [filename / Q{N}] â€” [brief description].
     Logged as Change Candidate [K].
  ```
- Proceed to 4a for the next question.

### 4c â€” Mid-loop commands
...
Use these banks to populate the question queue for each topic. Do not ask all questions â€” select the most valuable based on what is already known.
...
- What is the central dramatic question â€” the one sentence the whole book answers?
- What is the emotional promise to the reader? (e.g. catharsis, wonder, dread)
- What does the ending feel like â€” not what happens, but how it feels?
- What is the inciting incident and why does it destabilise the protagonist's world?
- What genre conventions does this story follow â€” and which does it subvert?
...
- What is this character's core wound â€” the thing that happened before the story began?
...
- What is the moment this character could change â€” and do they?
...
- Who in the cast challenges this character most â€” and why?
...
### `world-building`
- What is the single most important rule of this world â€” and what happens when it breaks?
...
- What does this world cost its inhabitants â€” what is the everyday price of living here?

### `locations`
- What does this place smell like, sound like, feel like underfoot?
- What is the emotional register of this location â€” what do people feel when they enter?
...
- How do the narrative voices differ â€” what does each character notice that the others miss?
- Where does information asymmetry create the most dramatic tension?
- Are there events in the story that no current POV character can witness â€” how do you handle them?- What is each POV characterâ€™s greatest blind spot â€” what are they systematically wrong about?
- Does the narrative distance (intimate vs. distant) stay consistent, or does it shift â€” and if it shifts, where and why?
- What does the reader know that the POV character doesnâ€™t, and when does that asymmetry close?
- Is there a character whose POV is deliberately withheld â€” what is the narrative cost and gain of that choice?
- How do you prevent reader confusion when switching POV â€” what are the grounding cues for each voice?
### `research`
...
- Is there an expert reader (a nurse, a sailor, a historian) who would catch errors â€” what would they flag?
- What is a piece of authentic texture â€” a smell, a procedure, a sound â€” that only someone who was there would know?
- Is there a commonly held belief about this subject that is actually wrong â€” one the story could use or subvert?
- What is the emotional texture of the real-world domain youâ€™re drawing from? Not just facts, but how it feels to live inside it?
- Which scene depends most heavily on research being correct â€” what breaks if you get it wrong?
- Are there legal, ethical, or sensitivity implications to how you portray this domain?

### `timeline`
- How much time does the story span â€” hours, months, decades?
- Are there time jumps â€” and what is revealed vs. hidden in the gap?
...
- Which term is most likely to be used inconsistently across a long draft â€” what are its variant forms?
- Are there terms that appear in dialogue â€” do they feel natural in a characterâ€™s mouth, or too technical, formal, or anachronistic?
- Which invented terms are load-bearing (plot or world-rule depends on their exact meaning) vs. flavour-only?
- Is there a term that feels forced or coined-for-the-sake-of-it â€” could it be replaced with something more organic?
- Are there terms borrowed from a real-world source (another language, a technical field) that carry unintended connotations?

### Wildcard bank

Draw from this bank once the topicâ€™s core question gaps are covered. These apply to any topic and are designed to surface unexpected angles.

- If this story never got published and only you would ever read it, what would you add that youâ€™re currently holding back?
- What is the most dangerous choice your protagonist could make right now â€” and what would it cost them?
...
- Which supporting character is quietly trying to become the protagonist â€” should you let them?
- What would need to be true about the world for your ending to feel inevitable rather than just earned?
- What is the one thing the reader will carry out of this story years later â€” and are you building toward it deliberately?

---

## Step 5 â€” Session End
...
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  ...
  Questions: [N] asked
  Insights : [M] logged
  Conflicts: [K] flagged  â† omit line if K = 0
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”

Key findings:
  â€¢ [1-sentence insight]
  â€¢ [1-sentence insight]
  â€¢ ...  (max 6 bullets, highest-value only)

What do you want to do with these results?

  1  Save brainstorm notes      â€” write a notes file alongside the topic file
  2  Update [topic file]        â€” apply findings and resolve conflicts directly
                                  (only shown if SESSION.has_context = true)
  3  Both                       â€” save notes AND update the topic file
  4  Merge all topics           â€” combine notes from every topic covered this session
                                  into one cross-topic brainstorm file
                                  (only shown if the user used  switch  at least once)
  5  Cancel                     â€” discard everything, no files written
...
### Option 1 or 3 â€” Save brainstorm notes
...
```markdown
# Brainstorm Notes â€” [Topic]

<!-- Generated: [DATE] | Questions: [N] | Insights: [M] -->

## Session Summary

[2â€“3 sentence summary of what the session explored and what it surfaced]
...
**Derived insight**: [AI synthesis â€” what this means for the story]
...
<!-- Status: PENDING â†’ APPLIED / SKIPPED / EDITED (updated during Step 6 interactive review) -->
...
Confirm to the user: `âœ“ Saved: {path to notes file}`

### Option 2 or 3 â€” Update topic file

Apply the brainstorm findings to the existing topic file:

1. **For each Change Candidate**: show the proposed edit (old text â†’ new text) and ask the user to confirm before writing:
   ```
   Change [N] of [K]:
   File    : [filename]
   ...
   ```
   - `y` â€” apply as shown
   - `n` â€” skip this change
   - `edit` â€” the user types the replacement text directly
...
- Confirm: `âœ“ Saved: {path} ([N] topics merged, [M] cross-topic connections noted.)`
...
- Leave all other `[NEEDS CLARIFICATION]` and `[PLACEHOLDER]` tokens intact â€” do not fill them with invented content.
- Confirm: `âœ“ Created: {path} ([N] sections populated from brainstorm, [M] left as template placeholders.)`
...
- **No fabrication.** Do not invent story facts. The AIâ€™s role is to ask, synthesise, and reflect â€” not to author.
- **Depth is binding.** When the depth limit is reached, surface the stopping point â€” do not slip in extra questions without prompting.
