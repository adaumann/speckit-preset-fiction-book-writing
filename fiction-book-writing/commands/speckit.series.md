...
- `audit` â€” run a full cross-book continuity check across all books (read-only)
- `audit [N-M]` â€” audit only books N through M (e.g. `audit 1-3`)
- `update [N]` â€” sync `series/series-bible.md` after book N is completed or drafted (e.g. `update 2`)
- `status` â€” display the series-level dashboard- `validate` — check that all carry-over variables are declared and reachable in all ending states (read-only)
...
|---|---|---|
| `init` | Before planning Book 1 â€” found the series | Yes (creates `series/series-bible.md`) |
| `audit` | Before planning a new book, or at any time | No â€” strictly read-only |
| `update [N]` | After a book's draft is finalized | Yes (series-bible.md + character Arc State tables) |
| `status` | Any time â€” live series-wide overview | No â€” strictly read-only || `validate` | After updating carry-over variables in series-bible.md | No — strictly read-only |
...
**Scope boundaries**:
- `speckit.series` does **not** check prose quality, character voice, or scene-level continuity within a single book â€” that is `speckit.continuity`.
- `speckit.series` does **not** check pre-draft spec/plan alignment within a single book â€” that is `speckit.analyze`.
...
- `init` â†’ go to **Mode: Init**
- `audit` or `audit N-M` â†’ go to **Mode: Audit**
- `update N` â†’ go to **Mode: Update**- `validate` → go to **Mode: Validate**
- `questionnaire` → go to **Mode: Questionnaire**
- `delta [N]` → go to **Mode: Delta**- `status` or *(empty)* â†’ go to **Mode: Status**

In all modes except `init`: locate `series/series-bible.md`. If not found, abort with:
```
âœ— series/series-bible.md not found.
  Run `speckit.series init` to create it, or run `speckit.plan` on the first
  non-standalone book â€” it will create the skeleton automatically.
```

Locate all book directories by scanning `specs/` for subdirectories matching the pattern `NNN-book-N-*/`. Parse the book number N from the directory name. Build the **Book Directory Map**: `{ book_number â†’ directory_path }`.
...
### Init Step 1 â€” Pre-flight check

If `series/series-bible.md` already exists, abort with:
```
âœ— series/series-bible.md already exists. To update series parameters, edit it directly.
  To sync after completing a book, use: speckit.series update [N]
```

### Init Step 2 â€” Gather series parameters

Ask the following questions. For each, accept a value from `$ARGUMENTS` if present; otherwise ask interactively:

1. **Series title** â€” the published series name (e.g. "The Shattered Key Chronicles")
2. **Total book count** â€” number of planned books, or "open series" if undetermined
3. **Genre** â€” primary genre (e.g. "Epic Fantasy", "Literary Fiction")
4. **Target audience** â€” age group and reader type (e.g. "Adult readers of character-driven fantasy")
5. **Overarching dramatic question** â€” one sentence; the series-level spine that must not be fully answered until the final book
6. **Overarching theme** â€” stated as a question (e.g. "Is the cost of justice ever too high to pay?")
7. **Series POV strategy** â€” how POV is handled across all books (e.g. "Consistent single-POV protagonist throughout" / "POV shifts between books as declared per constitution.md")
8. **Series tense** â€” consistent tense across all books, or notes on variances
9. **Series ending contract** â€” what the ending MUST deliver (not what happens â€” what it must feel like or resolve)

### Init Step 3 â€” Scaffold the series bible

Generate `series/series-bible.md` from `series-bible-template.md`. Populate all nine answers into their corresponding fields. Leave per-book fields (Books in Series table rows beyond Book 1, Character State Registry, Unresolved Series Threads) as template placeholders â€” they will be populated by `speckit.plan` and `speckit.series update`.

Create the `series/` directory if it does not exist.

Confirm:
```
âœ“ Created: series/series-bible.md
  Series     : [SERIES_TITLE]
  Books      : [count or open]
...
**Handoff â€” init mode only**: Display the full Book 1 sequence:
```
Next steps for Book 1 (in order):

  Step 1: speckit.constitution
    Set the story bible â€” style mode, prose profile, plot structure, POV strategy, tone.
...
Do not display the audit, update, or status handoffs at this stage â€” they are not relevant until a book exists.
...
### Audit Step 1 â€” Scope resolution
...
Scope          : Books [N]â€“[M] ([count] books)
```

If no range: audit all books present in the Book Directory Map.

### Audit Step 2 â€” Load all book assets

For each book in scope, load the following if they exist (skip silently if absent, note in report):

- `spec.md` â€” story brief, series position, character arcs
- `plan.md` â€” structure, open threads
- `.specify/memory/constitution.md` â€” story bible, `## IX. Series Context`
- `characters/` â€” all character profiles; specifically `## X. Series Arc State` tables
- `tasks.md` â€” for thread pay-off verification

Flag any book in scope that is missing `spec.md` as `INCOMPLETE` in the report â€” analysis for that book will be partial.

### Audit Step 3 â€” Run cross-book checks
...
3. Any mismatch between closing state of Book N and what Book N+1's character profile declares â†’ **CRITICAL**: `CHN-NNN: [CharacterName] state mismatch between Book N close and Book N+1 open â€” [field]: expected "[value]", found "[value]"`.
4. Any character whose `## X. Series Arc State` table is missing a row for a book where that character appears as a major arc â†’ **WARNING**: `CHN-NNN: [CharacterName] Series Arc State not logged for Book N`.
...
2. Any contradiction â†’ **CRITICAL**: `SCC-NNN: SC-[ID] violated in Book N, [chapter_id] â€” [detail]`.
3. Any `SC-NNN` row that has no `Established in` value â†’ **WARNING**: `SCC-NNN: SC-[ID] has no source â€” cannot verify establishment`.
...
2. Any violation â†’ **CRITICAL**: `STC-NNN: constraint violated in Book N â€” [detail]`.
...
2. If the pay-off book is drafted but contains no scene that delivers the thread's resolution:
   â†’ **WARNING**: `STR-NNN: ST-[ID] pay-off planned for Book N but not found in any drafted chapter â€” verify or update Planned pay-off`.
3. If the `Planned pay-off` column is empty â†’ **WARNING**: `STR-NNN: ST-[ID] has no planned pay-off assigned`.
4. If the `Introduced in` book has been drafted but the thread introduction scene cannot be located â†’ **WARNING**: `STR-NNN: ST-[ID] introduction scene not found in Book N drafts`.
...
â†’ **CRITICAL**: `SXC-NNN: Known contradiction SX-[ID] is OPEN â€” [conflict description] â€” resolve before drafting affected books`.

**F. Series Arc Pacing**

Load `## Per-Book Arc Contribution`. For each book row:

1. If both `Partial question answered` and `New question opened` are empty for any book with `Status: in-progress` or later â†’ **WARNING**: `ARC-NNN: Book N has no declared arc contribution â€” update series-bible.md ## Per-Book Arc Contribution`.
2. If any book except the final planned book has `New question opened` empty â†’ **WARNING**: `ARC-NNN: Book N closes questions but opens none â€” risk of series arc deflating prematurely`.
3. If the final planned book's `Partial question answered` is empty â†’ **WARNING**: `ARC-NNN: Final book arc contribution not declared`.
4. If the `## Series Ending Contract` field is `[SERIES_ENDING_CONTRACT]` or empty â†’ **WARNING**: `ARC-NNN: Series ending contract not written â€” reader promise is undefined`.
...
2. If `Last updated in` is earlier than the most recently drafted book where that entity appears â†’ **WARNING**: `NER-NNN: [EntityName] last updated in Book N but appears in later drafted books â€” verify canonical status is current`.

### Audit Step 4 â€” Output report

```
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  SERIES AUDIT REPORT
  Series  : [SERIES_TITLE]
  Scope   : Books [N]â€“[M] ([count] books audited)
  Date    : [YYYY-MM-DD]
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
...
|---|---|---|---|---|---|
| 1 | specs/001-book-1-â€¦/ | âœ“ | N chapters | âœ“ | 2 characters logged |
â€¦
...
### Update Step 2 â€” Character State Registry sync
...
1. Read the `After Book [N]` row. If it does not exist or contains only `[NEEDS CLARIFICATION]` values, prompt:
   > "[CharacterName] â€” Book [N] closing state is incomplete. Please provide:
   > - Relationship status:
   > - Physical state:
   > - Knowledge state:
   > - Arc position:
...
### Update Step 3 â€” Capture new world canon
...
### Update Step 4 â€” Capture new continuity constraints
...
> Describe each constraint â€” who knows what, what relationship has changed, what fact is now canon.
...
### Update Step 5 â€” New series threads
...
2. If new: prompt â€” "Is '[thread description]' a series-level thread that future books must pay off? (y/n)".
...
### Update Step 6 â€” Close resolved threads
...
- `y` â†’ set Status to `RESOLVED`
- `partial` â†’ keep `OPEN`, update Notes
- `n` â†’ keep `OPEN`, prompt to update `Planned pay-off` to a later book

### Update Step 7 â€” Update Books in Series table
...
### Update Step 8 â€” Update Per-Book Arc Contribution
...
### Update Step 9 â€” Confirm and report

```
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  SERIES BIBLE UPDATED â€” Book [N]: [BOOK_TITLE]
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
  ...
  Books in Series             : Book [N] status â†’ [drafted/published]

  âš ï¸  Review new SC / STC rows in series/series-bible.md before planning Book [N+1].
â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”
```

**Handoff â€” update mode only**: Display the following next-steps in order. The audit step is mandatory before briefing the next book:
...
Recommended next steps for Book [N+1]:
  ...
  Step 2: speckit.constitution
    ...
    series/series-bible.md â€” confirm or override per-book as needed.
    speckit.plan reads constitution.md as a required input; run this before plan.
...
| 2 | [TITLE]        | in-progress | 31,000     | â€”          | â€”              |
| 3 | [TITLE]        | planned     | â€”          | â€”          | â€”              |
```

Word count for each book: sum `actual_words` from all draft chapters in that book's `draft/` directory. If no drafts exist, show `â€”`.
...
```
âš ï¸  Action recommended: Run `speckit.series audit` to check cross-book continuity.
```
