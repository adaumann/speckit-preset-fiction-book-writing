...
Generate a node outline (`specs/[FEATURE_DIR]/outlines/NODE-NNN.md`) for one or more nodes. Outlines must be reviewed and approved before `speckit.implement` will draft the node.
...
Then:
1. Confirm `specs/[FEATURE_DIR]/flowmap.md` and `.speckit/memory/constitution.md` exist.
...
**Goal**: Generate structured, decision-complete node outlines in `specs/[FEATURE_DIR]/outlines/` that give the author all the information needed to approve the node before drafting begins.
...
2. **Load source documents**:
   ...
   - **Required**: `.speckit/memory/constitution.md` (mechanic schemas, hook types, world rules, tone, target platform)
