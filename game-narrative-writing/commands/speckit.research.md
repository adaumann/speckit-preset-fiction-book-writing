...
- For `add` mode: create it from `templates/research-template.md`. Populate `## Research Scope` from `specs/spec.md` and `.speckit/memory/constitution.md` if present (setting, technical domains, reference games, cultural context). Emit: `✓ Created research.md from template. Proceeding to add first item.`
...
3. > `Does this change anything in specs/flowmap.md, the spec, or drafted nodes?`
...
✓ Resolved: R[NNN] — [Topic Name]
  ...
  [Flowmap impact: (note) — review specs/flowmap.md and consider running speckit.continuity if nodes are drafted]
...
- A world rule stated as fact that should be backed by explicit design decision in `specs/world-building.md` or `.speckit/memory/constitution.md`
...
|-------|------------------------------------|-------------------|-------------------|
| R003  | Submarine dive order protocols     | NODE-2003         | ⚠️ HIGH           |
| R007  | Soviet coup announcement sequence  | NODE-1004         | ⚠️ HIGH           |
