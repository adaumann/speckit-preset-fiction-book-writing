...
9. **Report**:
   ...
   ```
   ⚠️ plan.md may need updating — choice targets or dialogue options changed. Run speckit.analyze to verify branch integrity.
   ```

   If glossary, location, or dialogue tree changes made: note:
   ```
   ⚠️ Glossary/Location/Dialogue changes made. Run speckit.continuity --check glossary,locations,dialogue to validate cross-node consistency.
   ```
