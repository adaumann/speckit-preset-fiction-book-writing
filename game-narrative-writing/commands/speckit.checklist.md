...
- `.speckit/memory/constitution.md` — craft rules, POV, prohibited phrases, tone, Prose Style Mode (Section VII)
...
- `specs/characters/` — NPC profiles for trust range and dialogue register checks
- `outlines/[NODE_ID].md` — node outline for outline-gate compliance check
- `specs/flowmap.md` — for upstream path analysis (NR-006)
...
For each node, evaluate all items in the four sections below using `templates/checklist-template.md` as the output structure. Mark each item ✅ / ❌ / ⚠️.
