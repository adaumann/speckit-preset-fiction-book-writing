---
description: Validate and compile drafted node files. Runs engine compiler, structural unit tests, and a self-correction loop up to 3 times before escalating to the user.
handoffs:
...
Validate one or more drafted node files and run the self-correction loop until they are clean.
